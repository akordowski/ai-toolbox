...
using SKToolCallBehavior = Microsoft.SemanticKernel.Connectors.OpenAI.ToolCallBehavior;
...
public sealed class AzureOpenAIPromptExecutionSettingsMapper : IPromptExecutionSettingsMapper
{
    public PromptExecutionSettings Map(PromptExecutionOptions options)
    {
        ...
        Verify.ThrowIfNotType<AzureOpenAIPromptExecutionOptions>(options);

        var opt = (AzureOpenAIPromptExecutionOptions)options;
        var toolCallBehavior = opt.ToolCallBehavior switch
        {
            ToolCallBehavior.AutoInvokeKernelFunctions => SKToolCallBehavior.AutoInvokeKernelFunctions,
            ToolCallBehavior.EnableKernelFunctions => SKToolCallBehavior.EnableKernelFunctions,
            null => null,
